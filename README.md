...
* **Save system state (xls)**. Allows the user to save a snapshot of the trajectory as xls file for later use. The snapshot can be reloaded as initial state by selecting "Manually input initial state" under "Options". Upon selecting, the user is prompted to input the time of the snapshot to be saved.
...
* **Kymograph**. 
* **Mean gene expression p(t)**. Plots a trajectory of p(t) against time. If there are two signals, the plot will contain two lines on the same plot.
* **Population fractions pij(t) (binary)**.
* **Spatial auto-correlation I(t)**. Same as above, for I(t).
* **Spatial auto-correlation Theta(t)**. Same as above, for Theta(t)/fN.
...
* **Pseudo-energy h(t)** (only for 1 signal, binary cell states). Plots the pseudo-energy h(t).
